...
\label{sec::CTFPAnalysis}
  ...
  dynamic on NA games admits solutions \fb{doesn't this follows from the existence of NE?}. Finally, we show that NA-CTFP reaches a fixed point when
...
%  
  ...
  \fb{why symbol $=:$? is it a definition?}
 ...
 to move from (\ref{eq::bilinear1}) to (\ref{eq::bilinear2}).
  ...
  \begin{theorem}
    ...
    BR^\mu(\alpha_\sigma^\mu)$ for almost all $t \geq 1$.
    \fb{why parameter $t$ if it does not appear anywhere in the statement?}
  \end{theorem}
...
%
  ...
  We now point out that, if we choose $w^{\mu \mu}$ to be zero for all $\mu$, then the final inequality \fb{which inequality?} yields that, for all agents $\mu$,
%
  \begin{equation} 
