...
\label{sec::CTFPAnalysis}
  ...
  dynamic on NA games admits solutions \fb{doesn't this follows from the existence of NE?} \ah{So the existence of the NE and the FP dynamic are independent of one another. The NE is a property of the game only, whereas FP is a separate differential equation (12 in the supplementary material). Whilst it is intuitive that the solutions to (S12) exists, on a technical sense we still have to prove it.}. Finally, we show that NA-CTFP reaches a fixed point when
...
%  
 ...
 to move from (\ref{eq::bilinear1}) to (\ref{eq::bilinear2}).
  ...
  \begin{lemma}
    ...
    BR^\mu(\alpha_\sigma^\mu(t))$ for almost all $t \geq 1$.
  \end{lemma}
...
%
  ...
  In the proof of Theorem \ref{thm::NACTFPtoFixed} it can be seen that, if we choose $w^{\mu \mu}$ to be zero for all $\mu$, then the fixed point corresponds to an NE. In particular Eqn. (26) in the supplementary materal corresponds to the NE condition when $w^{\mu \mu} = 0$. This leads to the next result.
%
...
\section{NA-CTFP achieves no regret}
  ...
  distribution over both the actions and the references.  Consider, then, a
  ...
  $(\refmu)_{i = 1}^N$, $\mathcal{D} =
  (\mathcal{D}^1, ..., \mathcal{D}^N)$ is such that
